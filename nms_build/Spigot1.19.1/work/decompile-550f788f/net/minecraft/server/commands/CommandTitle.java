...
public class CommandTitle {
    ...
    private static int clearTitle(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection) {
        ...
        if (collection.size() == 1) {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.title.cleared.single", ((EntityPlayer) collection.iterator().next()).getDisplayName()), true);
        } else {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.title.cleared.multiple", collection.size()), true);
        }
    ...
    private static int resetTitle(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection) {
        ...
        if (collection.size() == 1) {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.title.reset.single", ((EntityPlayer) collection.iterator().next()).getDisplayName()), true);
        } else {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.title.reset.multiple", collection.size()), true);
        }
    ...
    private static int showTitle(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection, IChatBaseComponent ichatbasecomponent, String s, Function<IChatBaseComponent, Packet<?>> function) throws CommandSyntaxException {
        ...
        if (collection.size() == 1) {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.title.show." + s + ".single", ((EntityPlayer) collection.iterator().next()).getDisplayName()), true);
        } else {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.title.show." + s + ".multiple", collection.size()), true);
        }
    ...
    private static int setTimes(CommandListenerWrapper commandlistenerwrapper, Collection<EntityPlayer> collection, int i, int j, int k) {
        ...
        if (collection.size() == 1) {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.title.times.single", ((EntityPlayer) collection.iterator().next()).getDisplayName()), true);
        } else {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.title.times.multiple", collection.size()), true);
        }
