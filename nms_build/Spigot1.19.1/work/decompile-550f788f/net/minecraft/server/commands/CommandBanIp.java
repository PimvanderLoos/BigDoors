...
public class CommandBanIp {
    ...
    private static int banIp(CommandListenerWrapper commandlistenerwrapper, String s, @Nullable IChatBaseComponent ichatbasecomponent) throws CommandSyntaxException {
        ...
        if (ipbanlist.isBanned(s)) {
            ...
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.banip.success", s, ipbanentry.getReason()), true);
            if (!list.isEmpty()) {
                commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.banip.info", list.size(), EntitySelector.joinNames(list)), true);
            }
            ...
            while (iterator.hasNext()) {
                ...
                entityplayer.connection.disconnect(IChatBaseComponent.translatable("multiplayer.disconnect.ip_banned"));
            }
