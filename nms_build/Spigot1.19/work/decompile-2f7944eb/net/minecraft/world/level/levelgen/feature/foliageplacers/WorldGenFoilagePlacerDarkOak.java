...
public class WorldGenFoilagePlacerDarkOak extends WorldGenFoilagePlacer {
    ...
    @Override
    protected void createFoliage(VirtualLevelReadable virtuallevelreadable, BiConsumer<BlockPosition, IBlockData> biconsumer, RandomSource randomsource, WorldGenFeatureTreeConfiguration worldgenfeaturetreeconfiguration, int i, WorldGenFoilagePlacer.a worldgenfoilageplacer_a, int j, int k, int l) {
        BlockPosition blockposition = worldgenfoilageplacer_a.pos().above(l);
        boolean flag = worldgenfoilageplacer_a.doubleTrunk();

        if (flag) {
            this.placeLeavesRow(virtuallevelreadable, biconsumer, randomsource, worldgenfeaturetreeconfiguration, blockposition, k + 2, -1, flag);
            this.placeLeavesRow(virtuallevelreadable, biconsumer, randomsource, worldgenfeaturetreeconfiguration, blockposition, k + 3, 0, flag);
            this.placeLeavesRow(virtuallevelreadable, biconsumer, randomsource, worldgenfeaturetreeconfiguration, blockposition, k + 2, 1, flag);
            if (randomsource.nextBoolean()) {
                this.placeLeavesRow(virtuallevelreadable, biconsumer, randomsource, worldgenfeaturetreeconfiguration, blockposition, k, 2, flag);
            }
        } else {
            this.placeLeavesRow(virtuallevelreadable, biconsumer, randomsource, worldgenfeaturetreeconfiguration, blockposition, k + 2, -1, flag);
            this.placeLeavesRow(virtuallevelreadable, biconsumer, randomsource, worldgenfeaturetreeconfiguration, blockposition, k + 1, 0, flag);
        }
