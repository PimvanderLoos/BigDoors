...
public class ItemArmorStand extends Item {
        ...
        if (enumdirection == EnumDirection.DOWN) {
            ...
            if (world.noCollision((Entity) null, axisalignedbb) && world.getEntities((Entity) null, axisalignedbb).isEmpty()) {
                if (world instanceof WorldServer) {
                    ...
                    Consumer<EntityArmorStand> consumer = EntityTypes.appendCustomEntityStackConfig((entityarmorstand) -> {
                    }, worldserver, itemstack, itemactioncontext.getPlayer());
    ...
    private void randomizePose(EntityArmorStand entityarmorstand, RandomSource randomsource) {
        Vector3f vector3f = entityarmorstand.getHeadPose();
        float f = randomsource.nextFloat() * 5.0F;
        float f1 = randomsource.nextFloat() * 20.0F - 10.0F;
