...
public class GameTestHarnessHelper {
    ...
    public void useBlock(BlockPosition blockposition, EntityHuman entityhuman, MovingObjectPositionBlock movingobjectpositionblock) {
        ...
        iblockdata.use(this.getLevel(), entityhuman, EnumHand.MAIN_HAND, movingobjectpositionblock);
    }
    ...
    public void assertEntityPresent(EntityTypes<?> entitytypes, Vec3D vec3d, Vec3D vec3d1) {
        ...
        if (list.isEmpty()) {
            throw new GameTestHarnessAssertionPosition("Expected " + entitytypes.toShortString() + " between ", new BlockPosition(vec3d), new BlockPosition(vec3d1), this.testInfo.getTick());
        }
    }
