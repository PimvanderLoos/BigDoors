...
public class CommandSchedule {
    ...
    private static final DynamicCommandExceptionType ERROR_CANT_REMOVE = new DynamicCommandExceptionType((object) -> {
        return IChatBaseComponent.translatable("commands.schedule.cleared.failure", object);
    });
    ...
    private static int schedule(CommandListenerWrapper commandlistenerwrapper, Pair<MinecraftKey, Either<CustomFunction, Collection<CustomFunction>>> pair, int i, boolean flag) throws CommandSyntaxException {
        if (i == 0) {
            ...
            ((Either) pair.getSecond()).ifLeft((customfunction) -> {
                ...
                commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.schedule.created.function", minecraftkey, i, j), true);
            }).ifRight((collection) -> {
                ...
                commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.schedule.created.tag", minecraftkey, i, j), true);
            });
    ...
    private static int remove(CommandListenerWrapper commandlistenerwrapper, String s) throws CommandSyntaxException {
        ...
        if (i == 0) {
            ...
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.schedule.cleared.success", i, s), true);
            return i;
        }
    }
}
