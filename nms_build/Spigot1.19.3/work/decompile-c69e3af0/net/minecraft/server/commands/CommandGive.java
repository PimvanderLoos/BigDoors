...
public class CommandGive {
    ...
    private static int giveItem(CommandListenerWrapper commandlistenerwrapper, ArgumentPredicateItemStack argumentpredicateitemstack, Collection<EntityPlayer> collection, int i) throws CommandSyntaxException {
        ...
        if (i > k) {
            commandlistenerwrapper.sendFailure(IChatBaseComponent.translatable("commands.give.failed.toomanyitems", k, argumentpredicateitemstack.createItemStack(i, false).getDisplayName()));
            return 0;
        } else {
            ...
            if (collection.size() == 1) {
                commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.give.success.single", i, argumentpredicateitemstack.createItemStack(i, false).getDisplayName(), ((EntityPlayer) collection.iterator().next()).getDisplayName()), true);
            } else {
                commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.give.success.single", i, argumentpredicateitemstack.createItemStack(i, false).getDisplayName(), collection.size()), true);
            }
