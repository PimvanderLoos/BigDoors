...
public class LightEngineThreaded extends LightEngine implements AutoCloseable {
    ...
    public CompletableFuture<IChunkAccess> retainData(IChunkAccess ichunkaccess) {
        ChunkCoordIntPair chunkcoordintpair = ichunkaccess.getPos();

        return CompletableFuture.supplyAsync(SystemUtils.name(() -> {
            super.retainData(chunkcoordintpair, true);
            return ichunkaccess;
        }, () -> {
            return "retainData: " + chunkcoordintpair;
        }), (runnable) -> {
            this.addTask(chunkcoordintpair.x, chunkcoordintpair.z, LightEngineThreaded.Update.PRE_UPDATE, runnable);
        });
    }
