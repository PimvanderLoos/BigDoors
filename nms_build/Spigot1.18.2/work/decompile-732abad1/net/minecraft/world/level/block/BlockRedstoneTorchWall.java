...
public class BlockRedstoneTorchWall extends BlockRedstoneTorch {
    ...
    public void animateTick(IBlockData iblockdata, World world, BlockPosition blockposition, Random random) {
        if ((Boolean) iblockdata.getValue(BlockRedstoneTorchWall.LIT)) {
            ...
            double d0 = 0.27D;
            double d1 = (double) blockposition.getX() + 0.5D + (random.nextDouble() - 0.5D) * 0.2D + 0.27D * (double) enumdirection.getStepX();
            double d2 = (double) blockposition.getY() + 0.7D + (random.nextDouble() - 0.5D) * 0.2D + 0.22D;
            double d3 = (double) blockposition.getZ() + 0.5D + (random.nextDouble() - 0.5D) * 0.2D + 0.27D * (double) enumdirection.getStepZ();
