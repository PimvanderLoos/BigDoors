...
public class WorldGenFeatureBasaltColumns extends WorldGenerator<WorldGenFeatureBasaltColumnsConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<WorldGenFeatureBasaltColumnsConfiguration> featureplacecontext) {
        ...
        if (!canPlaceAt(generatoraccessseed, i, blockposition.mutable())) {
            ...
            int j = worldgenfeaturebasaltcolumnsconfiguration.height().sample(random);
            boolean flag = random.nextFloat() < 0.9F;
            int k = Math.min(j, flag ? 5 : 8);
            int l = flag ? 50 : 15;
            boolean flag1 = false;
            Iterator iterator = BlockPosition.randomBetweenClosed(random, l, blockposition.getX() - k, blockposition.getY(), blockposition.getZ() - k, blockposition.getX() + k, blockposition.getY(), blockposition.getZ() + k).iterator();
            ...
            while (iterator.hasNext()) {
                ...
                if (i1 >= 0) {
                    flag1 |= this.placeColumn(generatoraccessseed, i, blockposition1, i1, worldgenfeaturebasaltcolumnsconfiguration.reach().sample(random));
                }
            }
