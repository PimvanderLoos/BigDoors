...
public class BlockColumnFeature extends WorldGenerator<BlockColumnConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<BlockColumnConfiguration> featureplacecontext) {
        ...
        for (int k = 0; k < i; ++k) {
            aint[k] = ((BlockColumnConfiguration.a) blockcolumnconfiguration.layers().get(k)).height().sample(random);
            j += aint[k];
        }
            ...
            for (l = 0; l < i; ++l) {
                ...
                if (i1 != 0) {
                    ...
                    for (int j1 = 0; j1 < i1; ++j1) {
                        generatoraccessseed.setBlock(blockposition_mutableblockposition, blockcolumnconfiguration_a.state().getState(random, blockposition_mutableblockposition), 2);
