        ...
        if (this.isRunning()) {
            ...
            try {
                ...
                this.recording = (Recording) SystemUtils.make(new Recording(configuration), (recording) -> {
                    ...
                    recording.setName("%s-%s-%s".formatted(environment.getDescription(), SharedConstants.getCurrentVersion().getName(), s));
                });
                Path path = Paths.get("debug/%s-%s.jfr".formatted(environment.getDescription(), s));
