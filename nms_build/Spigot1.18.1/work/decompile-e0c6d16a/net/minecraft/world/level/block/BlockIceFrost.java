...
public class BlockIceFrost extends BlockIce {
    ...
    public void tick(IBlockData iblockdata, WorldServer worldserver, BlockPosition blockposition, Random random) {
        if ((random.nextInt(3) == 0 || this.fewerNeigboursThan(worldserver, blockposition, 4)) && worldserver.getMaxLocalRawBrightness(blockposition) > 11 - (Integer) iblockdata.getValue(BlockIceFrost.AGE) - iblockdata.getLightBlock(worldserver, blockposition) && this.slightlyMelt(iblockdata, worldserver, blockposition)) {
            ...
            for (int j = 0; j < i; ++j) {
                ...
                if (iblockdata1.is((Block) this) && !this.slightlyMelt(iblockdata1, worldserver, blockposition_mutableblockposition)) {
                    worldserver.scheduleTick(blockposition_mutableblockposition, (Block) this, MathHelper.nextInt(random, 20, 40));
                }
            }

        } else {
            worldserver.scheduleTick(blockposition, (Block) this, MathHelper.nextInt(random, 20, 40));
        }
    }
