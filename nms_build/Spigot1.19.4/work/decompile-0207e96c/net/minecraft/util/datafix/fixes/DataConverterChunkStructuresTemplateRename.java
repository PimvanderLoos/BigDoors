...
public class DataConverterChunkStructuresTemplateRename extends DataFix {
    ...
    private Dynamic<?> fixChildren(Dynamic<?> dynamic) {
        return dynamic.update("Children", (dynamic1) -> {
            return dynamic.createList(dynamic1.asStream().map((dynamic2) -> {
                return this.fixTag(dynamic, dynamic2);
            }));
        });
    }
    ...
    private Dynamic<?> fixTag(Dynamic<?> dynamic, Dynamic<?> dynamic1) {
        ...
        if (DataConverterChunkStructuresTemplateRename.RENAMES.containsKey(s)) {
            Pair<String, ImmutableMap<String, String>> pair = (Pair) DataConverterChunkStructuresTemplateRename.RENAMES.get(s);
