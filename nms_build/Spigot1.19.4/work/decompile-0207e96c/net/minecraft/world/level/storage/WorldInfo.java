...
public class WorldInfo implements Comparable<WorldInfo> {
    ...
    private IChatBaseComponent createInfo() {
        if (this.isLocked()) {
            ...
            IChatMutableComponent ichatmutablecomponent = this.isHardcore() ? IChatBaseComponent.empty().append((IChatBaseComponent) IChatBaseComponent.translatable("gameMode.hardcore").withStyle((chatmodifier) -> {
                return chatmodifier.withColor(-65536);
            })) : IChatBaseComponent.translatable("gameMode." + this.getGameMode().getName());
