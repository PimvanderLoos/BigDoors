...
public class LongJumpToRandomPos<E extends EntityInsentient> extends Behavior<E> {
        ...
        if (d11 < 0.0D) {
            ...
            if (d12 > (double) this.maxJumpVelocity) {
                ...
                for (int k = 0; k < j - 1; ++k) {
                    ...
                    if (vec3d4 != null && !this.isClearTransition(entityinsentient, entitysize, vec3d4, vec3d5)) {
                        return null;
                    }
    ...
    private boolean isClearTransition(EntityInsentient entityinsentient, EntitySize entitysize, Vec3D vec3d, Vec3D vec3d1) {
