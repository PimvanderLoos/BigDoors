...
public class GameTestHarnessHelper {
    ...
    public void continuouslyUse(BlockPosition blockposition, EntityHuman entityhuman, MovingObjectPositionBlock movingobjectpositionblock) {
        BlockPosition blockposition1 = this.absolutePos(blockposition);
        IBlockData iblockdata = this.getLevel().getBlockState(blockposition1);
        EnumInteractionResult enuminteractionresult = iblockdata.use(this.getLevel(), entityhuman, EnumHand.MAIN_HAND, movingobjectpositionblock);
        ...
        if (!enuminteractionresult.consumesAction()) {
            ItemActionContext itemactioncontext = new ItemActionContext(entityhuman, EnumHand.MAIN_HAND, movingobjectpositionblock);
            ItemStack itemstack = entityhuman.getItemInHand(EnumHand.MAIN_HAND);

            if (entityhuman.isUsingItem()) {
                itemstack.onUseTick(this.getLevel(), entityhuman, entityhuman.getUseItemRemainingTicks());
            } else {
