...
public class CommandXp {
    ...
    private static int queryExperience(CommandListenerWrapper commandlistenerwrapper, EntityPlayer entityplayer, CommandXp.Unit commandxp_unit) {
        ...
        commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.experience.query." + commandxp_unit.name, entityplayer.getDisplayName(), i), false);
        return i;
    }
    ...
    private static int addExperience(CommandListenerWrapper commandlistenerwrapper, Collection<? extends EntityPlayer> collection, int i, CommandXp.Unit commandxp_unit) {
        ...
        if (collection.size() == 1) {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.experience.add." + commandxp_unit.name + ".success.single", i, ((EntityPlayer) collection.iterator().next()).getDisplayName()), true);
        } else {
            commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.experience.add." + commandxp_unit.name + ".success.multiple", i, collection.size()), true);
        }
    ...
    private static int setExperience(CommandListenerWrapper commandlistenerwrapper, Collection<? extends EntityPlayer> collection, int i, CommandXp.Unit commandxp_unit) throws CommandSyntaxException {
        ...
        if (j == 0) {
            ...
            if (collection.size() == 1) {
                commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.experience.set." + commandxp_unit.name + ".success.single", i, ((EntityPlayer) collection.iterator().next()).getDisplayName()), true);
            } else {
                commandlistenerwrapper.sendSuccess(IChatBaseComponent.translatable("commands.experience.set." + commandxp_unit.name + ".success.multiple", i, collection.size()), true);
            }
