...
public class CommandData {
    ...
    private static final DynamicCommandExceptionType ERROR_EXPECTED_VALUE = new DynamicCommandExceptionType((object) -> {
        return IChatBaseComponent.translatable("commands.data.modify.expected_value", object);
    });
    ...
    private static List<NBTBase> stringifyTagList(List<NBTBase> list, Function<String, String> function) throws CommandSyntaxException {
        ...
        while (iterator.hasNext()) {
            ...
            list1.add(NBTTagString.valueOf((String) function.apply(s)));
        }
                ...
                while (iterator1.hasNext()) {
                    ...
                    biconsumer.accept(argumentbuilder1, (commanddata_a) -> {
                        return commanddata_c1.wrap(net.minecraft.commands.CommandDispatcher.literal("string"), (argumentbuilder2) -> {
                            ...
                            })).then(((RequiredArgumentBuilder) net.minecraft.commands.CommandDispatcher.argument("start", IntegerArgumentType.integer(0)).executes((commandcontext) -> {
                                return manipulateData(commandcontext, commanddata_c, commanddata_a, stringifyTagList(resolveSourcePath(commandcontext, commanddata_c1), (s) -> {
                                    return s.substring(IntegerArgumentType.getInteger(commandcontext, "start"));
                                }));
                            })).then(net.minecraft.commands.CommandDispatcher.argument("end", IntegerArgumentType.integer(0)).executes((commandcontext) -> {
                                return manipulateData(commandcontext, commanddata_c, commanddata_a, stringifyTagList(resolveSourcePath(commandcontext, commanddata_c1), (s) -> {
                                    return s.substring(IntegerArgumentType.getInteger(commandcontext, "start"), IntegerArgumentType.getInteger(commandcontext, "end"));
                                }));
                            }))));
                        });
                    });
